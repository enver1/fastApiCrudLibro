...
def obtener_idioma_por_id(id, db: Session = Depends(get_db)):
    ...
    raise HTTPException(status_code=404, detail='User not Found!!')
#edit
@ruta_idioma.put("/idioma/editar/{id}")
def actualizar_idioma(id, idioma_esquema: IdiomaDatos, db: Session = Depends(get_db)):
    obj = crud.editar_idioma(db=db, id=id, idioma_esquema=idioma_esquema)
    if obj:
        return obj
